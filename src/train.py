...
class Trainer:
    ...
    def train(
        ...
        for step in range(max_steps):
            ...
            if step % self.eval_every == 0 or self.is_last_step:
                self.evaluate(step)

            # evaluate HellaSwag every once in a while
            if ((step > 0 and step % self.eval_every == 0) or self.is_last_step) and (not use_torch_compile):
    ...
    def generate(self):
        ...
        while gen_tokens.shape[-1] <= max_tokens:
            with torch.no_grad():
                with torch.autocast(device_type=self.device_type, dtype=torch.bfloat16):
                    logits, loss = self.model(gen_tokens)    # (n_return_seq, n, vocab_size)
...
def get_args():
    ...
    parser.add_argument("--total_batch_size", type=int, default=524288)    # =2^19 tokens/step update, (~0.5M tokens used in openai gpt3 paper)
    ...
    parser.add_argument("--context_length", type=int, default=1024)    # max sequence length
    parser.add_argument("--num_layers", type=int, default=12)
    parser.add_argument("--embd_size", type=int, default=768)
    parser.add_argument("--num_heads", type=int, default=12)
    parser.add_argument("--max_lr", type=float, default=6e-4 * 2)
    parser.add_argument("--min_lr", type=float, default=6e-4 * 2 * 0.1)
    parser.add_argument("--warmup_steps", type=int, default=715)
    parser.add_argument("--weight_decay", type=float, default=0.1)
    parser.add_argument("--max_steps", type=int, default=19073 * 1)    # 10^10 / 2^19 ~ 19073 for 1 epoch
...
def main():
    ...
    if ddp:
        ...
        # master process will do printing, logging, checkpointing, etc.
    ...
    if master_process:
        print(f'desired batch size: {args.total_batch_size}')
    ...
    # create GPT model
    gpt_config = GPTConfig(vocab_size=50304,   # 50304 (nice number, lots of powers of 2s) used instead of 50257 (bad, odd number)
    ...
    trainer = Trainer(model, optimizer, train_loader, val_loader, token_encoder, args.eval_every, grad_accum_steps, 
                      ddp, ddp_rank, ddp_world_size, device, logpath)
    trainer.train(args.max_steps, args.warmup_steps, args.max_lr, args.min_lr)
